...
void decrypt_file(FILE* fp, char* key) {
    ...
    /*
    OLA MONITOR, essad duas linhas abaixo foram adicionadas para corrigir o problema
    o código original criptografava o ponteiro "fp", nesse a criptografia é no
    "formated_cipher".
    */
    format_plaintext(fp);
    FILE* formated_cipher = fopen("formated_input.txt", "r");
    ...
    int i = 0;  // Used to cycle over key.
    while (((file_char = fgetc(formated_cipher)) != EOF)) {
        if (isalpha(file_char)) {
            char plain_char;
            // m_i = (c_i - k_i) mod 26.
